...
The dashboard now reports how many live sources successfully returned observations. Missing or temporarily unavailable feeds are excluded from the aggregate score instead of being interpreted as a market signal. Each FRED series is fetched independently with retries; one failed series no longer prevents the rest of the dashboard from updating.

The 10Y–2Y curve is now intended to use FRED's daily spread series directly when included in a future extension; the current dashboard continues to show the derived curve from aligned daily observations. FRED confirms that T10Y2Y is a daily series calculated from the Treasury's 10Y and 2Y constant-maturity yields. See the FRED documentation for details.
