...
for k in FRED:
    try:
        rows=fred_csv(k)
        series[k]=rows
        source_status[k]={"source":"FRED","ok":bool(rows),"observations":len(rows),"last_date":rows[-1][0] if rows else None,"error":None if rows else "No observations returned"}
    except Exception as e:
        series[k]=[]
        source_status[k]={"source":"FRED","ok":False,"observations":0,"last_date":None,"error":str(e)[:180]}
try:
    series["DXY"]=yahoo_dxy()
    source_status["DXY"]={"source":"Yahoo Finance","ok":bool(series["DXY"]),"observations":len(series["DXY"]),"last_date":series["DXY"][-1][0] if series["DXY"] else None,"error":None if series["DXY"] else "No observations returned"}
except Exception as e:
    series["DXY"]=[]
    source_status["DXY"]={"source":"Yahoo Finance","ok":False,"observations":0,"last_date":None,"error":str(e)[:180]}
...
def curve_rows(a,b):
    # Align by date; missing observations are simply omitted.
    if not a or not b: return []
    bm={d:v for d,v in b if v is not None}
    return [(d,va-bm[d]) for d,va in a if d in bm and va is not None]

def safe_weighted(parts):
...
# Bucket scores. Missing buckets are excluded from the aggregate rather than treated as stress or normal.
infl=score_metric("WTI Crude",series["DCOILWTICO"]) if series["DCOILWTICO"] else None
rates=safe_weighted([(score_metric("10Y Treasury",rates10),0.45),(score_metric("30Y Treasury",rates30),0.55)])
labor=safe_weighted([(score_metric("Unemployment",u),0.65),(sahm_heat if sahm is not None else None,0.35)])
credit=safe_weighted([(score_metric("HY Credit Spread",series["BAMLH0A0HYM2"]),0.70),(score_metric("IG Credit Spread",series["BAMLC0A0CM"]),0.30)])
usd=score_metric("DXY",series["DXY"]) if series["DXY"] else None

# Credit and labor receive the largest weights, but the final score is renormalized if a bucket is unavailable.
parts=[(infl,0.18),(rates,0.23),(labor,0.25),(credit,0.26),(usd,0.08)]
valid=[(v,w) for v,w in parts if v is not None]
score=round(sum(v*w for v,w in valid)/sum(w for _,w in valid)) if valid else 50
...
elif wti is None:add_alert("Oil data unavailable","WTI data could not be retrieved; the dashboard will not treat missing data as a stress signal.",3)
elif wti30 is None:add_alert("Oil trend unavailable",f"WTI is ${wti:.2f}/bbl, but a 30-day comparison is unavailable.",3)
...
elif dxy is None:add_alert("DXY data unavailable","DXY data could not be retrieved; the dashboard will not treat missing data as a stress signal.",3)
elif dxy30 is None:add_alert("DXY trend unavailable",f"DXY is {dxy:.2f}, but a 30-day comparison is unavailable.",3)
...
out={
 ...
 "score":score,
 "dataHealth":{"live":sum(1 for x in source_status.values() if x["ok"]),"total":len(source_status),"status":"green" if all(x["ok"] for x in source_status.values()) else "yellow" if any(x["ok"] for x in source_status.values()) else "red","sources":source_status},
 "accent":"#32d296" if score<25 else "#f5c451" if score<50 else "#fb923c" if score<70 else "#f87171",
 "regime":reg,"regimeDesc":desc,
 "chips":[f"Rates {round(rates) if rates is not None else '—'}/100",f"Credit {round(credit) if credit is not None else '—'}/100",f"Labor {round(labor) if labor is not None else '—'}/100",f"Oil {round(infl) if infl is not None else '—'}/100",f"DXY {round(usd) if usd is not None else '—'}/100"],
 "gauges":[{"name":"Inflation / oil","score":round(infl) if infl is not None else None},{"name":"Rates / term premium","score":round(rates) if rates is not None else None},{"name":"Labor / recession","score":round(labor) if labor is not None else None},{"name":"Credit stress","score":round(credit) if credit is not None else None},{"name":"USD / liquidity","score":round(usd) if usd is not None else None}],
 ...
 "rates":[
    ...
    metric("10Y − 2Y",curve10_2,"pp","Curve context; positive does not automatically mean bullish or bearish.",2,heat=(clamp(25+max(0,abs(last(curve10_2)[1]-0.5))*20) if last(curve10_2)[1] is not None else 50)),
    metric("30Y − 10Y",curve30_10,"pp","Long-end term-premium signal.",2,heat=(clamp(25+max(0,last(curve30_10)[1]-0.3)*80) if last(curve30_10)[1] is not None else 50))
 ],
 "macro":[
